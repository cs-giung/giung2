...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        out = self.conv1(self.relu1(self.norm1(x,   **kwargs), **kwargs), **kwargs)
        out = self.conv2(self.relu2(self.norm2(out, **kwargs), **kwargs), **kwargs)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv(in_channels=planes, out_channels=planes,
                          kernel_size=3, stride=stride, padding=1, **kwargs)
        self.norm3 = norm(num_features=planes)
        self.relu3 = relu()
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        out = self.conv1(self.relu1(self.norm1(x,   **kwargs), **kwargs), **kwargs)
        out = self.conv2(self.relu2(self.norm2(out, **kwargs), **kwargs), **kwargs)
        out = self.conv3(self.relu3(self.norm3(out, **kwargs), **kwargs), **kwargs)
        out = out + self.shortcut(x, **kwargs)
        return out
...
class LastBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        out = self.relu1(self.norm1(x, **kwargs), **kwargs)
        return out
...
class PreResNet(nn.Module):
    ...
    def __init__(
        ...
        _layers += [
            LastBlock(
                self.in_planes * (2 ** idx) * self.widen_factor * self.block.expansion,
                self.conv,
                self.norm,
                self.relu,
                **kwargs
...
def build_preresnet_backbone(cfg: CfgNode) -> nn.Module:
